    ...
    const showAttachments = async () => {
        await dialog({
            ...
            message: (
                <SizeGetter
                    render={({ width }) => (
                        <AttachmentsModalBody attachments={attachments} availableWidthInPx={width} />
                    )}
                />
            ),
            modalSize: "lg",
...
function AttachmentsModalBody({
    ...
    const hasNotFound = attachments.filter((attachment) => attachment.Source === "NotFound").length > 0;
