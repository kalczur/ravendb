        ...
        module: {
            rules: [
                ...
                },
                {
                    ...
                    include: includeFileFromComponentsWithoutTanstack,
                    use: [
                        {
                            loader: "thread-loader",
                        },
...
function getPlugins({ isProductionMode, isWatchMode }) {
    ...
    if (!isWatchMode) {
        // it runs on file change and it's slow - skip in watch mode
        plugins.push(
...
function includeFileFromComponentsWithoutTanstack(resourcePath) {
    if (!resourcePath.includes("components")) {
        return false;
    }

    try {
        const src = fs.readFileSync(resourcePath, "utf8");

        if (src.includes("@tanstack/react-table") || src.includes("@tanstack/react-virtual")) {
            return false;
        }
